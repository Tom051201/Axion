...
namespace Axion {
	...
	Silica::WidgetPtr ShaderImportModal::getWidget(Silica::FontAtlas* font, std::function<void()> onClose) {
		m_font = font;
		m_onClose = onClose;
	...
	void ShaderImportModal::rebuildUI_Internal() {
		...
		// -- Header --
		contentBox->addSlot({ {0,0}, Silica::MakeWidget<Silica::STextBlock>({.text = "Import Shader Asset", .font = m_font}) });
		...
		auto batchInput = Silica::MakeWidget<Silica::SBox>({
			.backgroundColor = Silica::Color(35, 35, 35, 255),
			.child = Silica::MakeWidget<Silica::SEditableText>({
				.initialText = std::to_string(m_batchTexturesCount), .font = m_font,
				...
				{ {0,0}, Silica::MakeWidget<Silica::SButton>({
					...
					.onClick = [this]() {
						...
						std::filesystem::path absPath = std::filesystem::exists(shaderDir) ?
							FileDialogs::openFile({ {"Shader Source", "*.hlsl;*.glsl"} }, shaderDir) :
							FileDialogs::openFile({ {"Shader Source", "*.hlsl;*.glsl"} }, ProjectManager::getProject()->getAssetsPath());
			...
			.slots = {
				...
				{ {0,0}, Silica::MakeWidget<Silica::SButton>({
					...
					.onClick = [this]() {
						std::filesystem::path shaderDir = ProjectManager::getProject()->getAssetsPath() / "shaders";
						std::filesystem::path absPath = std::filesystem::exists(shaderDir) ?
							FileDialogs::openFolder(shaderDir) : FileDialogs::openFolder(ProjectManager::getProject()->getAssetsPath());
