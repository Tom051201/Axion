...
namespace Axion {
	...
	Silica::WidgetPtr MeshImportModal::getWidget(Silica::FontAtlas* font, std::function<void()> onClose) {
		m_font = font;
		m_onClose = onClose;
	...
	void MeshImportModal::rebuildUI_Internal() {
		...
		// -- Header --
		contentBox->addSlot({ {0,0}, Silica::MakeWidget<Silica::STextBlock>({.text = "Import Mesh Asset", .font = m_font}) });
			...
			.slots = {
				...
				{ {0,0}, Silica::MakeWidget<Silica::SButton>({
					...
					.onClick = [this]() {
						std::filesystem::path meshDir = ProjectManager::getProject()->getAssetsPath() / "meshes";
						std::filesystem::path absPath = std::filesystem::exists(meshDir) ?
							FileDialogs::openFolder(meshDir) : FileDialogs::openFolder(ProjectManager::getProject()->getAssetsPath());
