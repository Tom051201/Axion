...
namespace Axion {
	...
	Silica::WidgetPtr TextureCubeImportModal::getWidget(std::function<void()> onClose) {
		m_onClose = onClose;
	...
	void TextureCubeImportModal::rebuildUI_Internal() {
		...
		// -- Header --
		contentBox->addSlot({ {0,0}, Silica::MakeWidget<Silica::STextBlock>({.text = "Import TextureCube Asset" }) });
			...
			.slots = {
				...
				{ {0,0}, Silica::MakeWidget<Silica::SButton>({
					...
					.onClick = [this]() {
						...
						std::filesystem::path absPath = FileDialogs::openFile({ {"Image File", "*.png;*.jpg;*.jpeg"} }, dir);
