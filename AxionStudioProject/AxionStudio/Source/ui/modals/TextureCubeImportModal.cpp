...
namespace Axion {
	...
	Silica::WidgetPtr TextureCubeImportModal::getWidget(Silica::FontAtlas* font, std::function<void()> onClose) {
		m_font = font;
		m_onClose = onClose;
	...
	void TextureCubeImportModal::rebuildUI() {
	...
	void TextureCubeImportModal::rebuildUI_Internal() {
		if (!m_uiRoot) return;
		...
		// -- Header --
		contentBox->addSlot({ {0,0}, Silica::MakeWidget<Silica::STextBlock>({.text = "Import TextureCube Asset", .font = m_font}) });
				...
				{ {0,0}, Silica::MakeWidget<Silica::SButton>({
					...
					.onClick = [this]() {
						...
						std::filesystem::path absPath = std::filesystem::exists(dir) ?
							FileDialogs::openFile({ {"Image File", "*.png;*.jpg;*.jpeg"} }, dir) :
