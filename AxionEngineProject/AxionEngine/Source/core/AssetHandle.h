...
//
//		AssetHandle() : path() {}
//		AssetHandle(const std::string& path) : path(path) {}
//
//		bool operator==(const AssetHandle& other) const { return path == other.path; }
//		bool operator!=(const AssetHandle& other) const { return !(*this == other); };
//	};
//
//}
...
//	struct hash<Axion::AssetHandle<T>> {
//		size_t operator()(const Axion::AssetHandle<T>& handle) const noexcept {
//			return std::hash<std::string>()(handle.path);
//		}
//	};
//}
//
...
//		template<typename FormatContext>
//		auto format(const Axion::AssetHandle<T>& handle, FormatContext& ctx) const {
//			return format_to(ctx.out(), "AssetHandle<{}>( {} )", typeid(T).name(), handle.path);
//		}
//	};
//}
//