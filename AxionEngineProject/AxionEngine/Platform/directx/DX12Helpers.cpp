...
namespace Axion {
	...
	DXGI_FORMAT DX12Helpers::toDX12ColorFormat(ColorFormat format) {
		switch (format) {
	...
	DXGI_FORMAT DX12Helpers::toDX12DepthStencilFormat(DepthStencilFormat format) {
		switch (format) {
	...
	D3D12_CULL_MODE DX12Helpers::toDX12CullMode(CullMode mode) {
		switch (mode) {
	...
	D3D12_COMPARISON_FUNC DX12Helpers::toDX12DepthComparisonFunction(DepthCompare function) {
		switch (function) {
	...
	D3D12_PRIMITIVE_TOPOLOGY_TYPE DX12Helpers::toDX12ToplogyType(PrimitiveTopology topology) {
		switch (topology) {
	...
	D3D12_PRIMITIVE_TOPOLOGY DX12Helpers::toDX12Topology(PrimitiveTopology topology) {
		switch (topology) {
	...
	DXGI_FORMAT DX12Helpers::ShaderDataTypeToDXGIFormat(ShaderDataType type) {
		switch (type) {
